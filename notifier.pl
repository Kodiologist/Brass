...
foreach ($db->query('select * from
        (select sn, email, checkin_code, first_d8, last_d8 from Subjects where active)
        left natural join
        (select sn, max(d8) as prev_wakeup_d8
            from WakeupTimes where submitted_t not null group by sn)')->hashes)
   {my %s = %$_;
    if ($s{first_d8} > $today)
       {email to => $s{email},
            subject => 'Decision-Making and Habits',
            msg => sprintf $welcome_fmt, sprintf $p{checkin_url_fmt}, $s{checkin_code};}
       ...
       {# Mark today as a missed day.
        ...
        # Kick the subject ouf the study, or just give them a
        # warning.
        if ($misses == 1)
           {if ($today == $s{last_d8})
               {$db->update('Subjects', {active => 0}, {sn => $s{sn}});
                email to => $s{email},
                   subject => 'Decision-Making and Habits is complete',
                   msg => q(You seem to have forgotten to check in for the Decision-Making and Habits study today. No worries, though: it was the last day anyway. You'll receive full credit.);}
            else
               {email to => $s{email},
                   subject => 'Did you forget to check in today?',
                   msg => q(You seem to have forgotten to check in for the Decision-Making and Habits study today. That's okay, but if you forget again, you'll be ejected from the study.);}}
        else # $misses > 1
           {$db->update('Subjects', {active => 0}, {sn => $s{sn}});
            email to => $s{email},
               subject => 'Did you forget to check in today?',
               msg => q(You seem to have forgotten to check in for the Decision-Making and Habits study today. That's your second strike, so I'm afraid you're out of the study. You'll still receive credit for the parts of the study you've completed.);}}}
